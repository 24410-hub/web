...
const GAS_PROXY_URL = "https://script.google.com/macros/s/AKfycbxk2RGY4iOh8rm7dAG4uZjiIGZ21mqWCiurk2YIaZOCXZdApN70LAA4KgkMknhQcnJP8g/exec";
...
async function callProxy(payload) {
  let res;
  try {
    // Content-Type을 text/plain으로 — application/json은 preflight 유발로 GAS가 HTML 리다이렉트 반환함
  ...
  try {
    ...
    if (text.trim().startsWith("<")) {
      // GAS가 HTML을 반환 = 배포 설정 문제
      return { ok: false, errorType: "parse",
        message: "⚠️ [배포 오류] Apps Script 배포 설정 확인 필요: 배포 → 새 배포 → 액세스: 모든 사용자(익명 포함)" };
    }
